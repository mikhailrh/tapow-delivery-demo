...
Earlier passes had a one-way kitchen-message sheet (write-only, vendor → customer, status-update bubbles only). Pass 4 turns it into a real two-way thread on the order, with a three-phase lifecycle, opt-in review capture, and image attach. Customer-side, the chat is **in-platform** — WhatsApp's role is reduced to its production reality: a notification rail carrying the receipt, system status events, and preview pills that deep-link into the actual conversation. The chat itself lives in [src/screens/OrderChatScreen.tsx](src/screens/OrderChatScreen.tsx).

### Where the surfaces live

- **WhatsAppScreen** = the notification rail. Keeps the receipt, system status events (accepted / cooking / ready / refund / delivered / etc.), the refund pill (anchored to `refund.requestedAt`), the apology-credit red box on rejection, the cancel-order pill while incoming, and the download-receipt pill after collection. Free-text vendor messages render here as a **slim preview pill** (`VendorPreviewPill`) — venue chip + truncated preview + "Tap to reply ›" — that calls `go({ name: "orderChat", orderId })`. Customer messages are never surfaced on this rail.
- **OrderChatScreen** = the in-platform chat home. Tapow-styled (not WhatsApp chrome). Header carries a labeled "Back to WhatsApp" control (code-commented as a demo-flow device — production re-entry is via WhatsApp notification, not an in-app button); below that, "Chat with [venue]" framing + the lifecycle status line + a "Receipt sent to your WhatsApp ✓" callback chip with a "View" link back to WhatsAppScreen. Then the scrollable thread (`Order.messages` only — system events stay on WhatsApp), the opt-in review affordance, the floating preview pill for unread vendor messages when scrolled away from the bottom, and the phase-aware composer with image attach.
...
- **NavContext** ([src/context/NavContext.tsx](src/context/NavContext.tsx)) — adds `| { name: "orderChat"; orderId: string }` to `Screen`. Routed through [App.tsx](src/App.tsx) `CustomerScreen`. Pushed from WhatsAppScreen preview pills; pops back via `back()`.
- **Three-phase lifecycle** — `getChatPhase(order)` returns `pre-pickup | in-transit | post-delivery | closed`. `POST_DELIVERY_CHAT_WINDOW_MS` (default 4h after collection) gates the feedback window; after it elapses (or on reject/cancel) the thread renders read-only on both sides. `lifecycleLabel(order)` returns the pinned status-line copy + tone, surfaced as `OrderStatusLine` on WhatsAppScreen, on OrderChatScreen's header, and as the header strip inside `VendorChatSheet`.
...
- **Seeded chat thread** — `seedFowlBoysChat` ([src/context/OrdersContext.tsx](src/context/OrdersContext.tsx)) attaches a canonical "out of crack fries → sub waffle fries → yes please" exchange to the most-recent collected FowlBoys order so the vendor's Messages surface (and the customer's OrderChatScreen, reached via the WhatsApp preview pill) both land populated for the pitch. `SEED_FLAG_SUFFIX` bumped to `everSeeded.v4` to re-seed existing browsers with the new `messages` shape.
