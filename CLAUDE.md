...
Earlier passes had a one-way kitchen-message sheet (write-only, vendor → customer, status-update bubbles only). Pass 4 turns it into a real two-way thread on the order, with a three-phase lifecycle, opt-in review capture, and image attach.
...
- **Three-phase lifecycle** — `getChatPhase(order)` returns `pre-pickup | in-transit | post-delivery | closed`. `POST_DELIVERY_CHAT_WINDOW_MS` (default 4h after collection) gates the feedback window; after it elapses (or on reject/cancel) the thread renders read-only on both sides. `lifecycleLabel(order)` returns the pinned status-line copy + tone, surfaced as `OrderStatusLine` on WhatsAppScreen and as the header strip inside `VendorChatSheet`.
- **Customer surface** — [src/screens/WhatsAppScreen.tsx](src/screens/WhatsAppScreen.tsx) merges `statusUpdates` (minus the first "received" event, which is rolled into the receipt bubble) and `messages` into a single time-sorted `timeline`, drip-fed via `bubbleCount`. Phase-aware `ChatInput` at the bottom (placeholder shifts per phase, disabled with explanatory hint when phase === "closed"). Refund pill now anchors on `refund.requestedAt` instead of sniffing message text — so refund copy can change without breaking the pill.
...
- **Preview affordance** — WhatsAppScreen tracks scroll position; when the customer is scrolled away from the bottom and a new vendor message arrives, a floating pill appears above the input ("[VenueName] · short preview ↓"). Tap → smooth-scroll to bottom + clear unread. `lastSeenAt` initializes to `Date.now()` at mount so existing seeded vendor messages don't trigger the banner.
- **Seeded chat thread** — `seedFowlBoysChat` ([src/context/OrdersContext.tsx](src/context/OrdersContext.tsx)) attaches a canonical "out of crack fries → sub waffle fries → yes please" exchange to the most-recent collected FowlBoys order so the vendor's Messages surface lands populated for the pitch. `SEED_FLAG_SUFFIX` bumped to `everSeeded.v4` to re-seed existing browsers with the new `messages` shape.
