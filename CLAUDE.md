...
- **Seeded chat thread** — `seedFowlBoysChat` ([src/context/OrdersContext.tsx](src/context/OrdersContext.tsx)) attaches a canonical "out of crack fries → sub waffle fries → yes please" exchange to the most-recent collected FowlBoys order so the chat surfaces (both the customer's `OrderChatScreen` and the vendor's Messages sheet) land populated for the pitch. `SEED_FLAG_SUFFIX` bumped to `everSeeded.v4` to re-seed existing browsers with the new `messages` shape.
